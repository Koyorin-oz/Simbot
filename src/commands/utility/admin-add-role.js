...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("admin-add-role")
    .setDescription("Admin: ajoute un role a un membre")
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addUserOption((o) => o.setName("membre").setDescription("Membre cible").setRequired(true))
    .addRoleOption((o) => o.setName("role").setDescription("Role a ajouter").setRequired(true)),
  async execute(client, interaction) {
    ...
    if (member.roles.cache.has(role.id)) {
      await interaction.reply({ content: `${member.user.tag} a deja le role ${role}.`, flags: MessageFlags.Ephemeral });
      return;
    }

    await member.roles.add(role.id, `Ajout par ${interaction.user.tag}`);
    await interaction.reply(`Role ${role} ajoute a ${member.user.tag}.`);
  }
};
