...
const { removeSC, removeSP, removeLP } = require("./_economyAdminUtils");
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription("Admin: retire des ressources a un membre")
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addSubcommand((s) =>
      s
        .setName("money")
        .setDescription("Retirer des Simba Coins")
    ...
    .addSubcommand((s) =>
      s
        .setName("sp")
        .setDescription("Retirer des Simba Points")
        .addUserOption((o) => o.setName("membre").setDescription("Membre cible").setRequired(true))
        .addIntegerOption((o) => o.setName("montant").setDescription("Montant").setMinValue(1).setRequired(true))
    )
    .addSubcommand((s) =>
      s
        .setName("lp")
        .setDescription("Retirer des Level Points")
        .addUserOption((o) => o.setName("membre").setDescription("Membre cible").setRequired(true))
        .addIntegerOption((o) => o.setName("montant").setDescription("Montant").setMinValue(1).setRequired(true))
    ),
  async execute(client, interaction) {
    ...
    if (sub === "money") {
      ...
      await interaction.editReply(
        `Retrait de ${formatSC(amount)} Simba Coins a ${member.tag}. Nouveau solde: ${formatSC(updated.simbaCoins)} SC.`
      );
      return;
    }
    if (sub === "sp") {
      ...
      const syncWarning =
        syncStatus && !syncStatus.ok
          ...
          : "";
      await interaction.editReply(
        `Retrait de ${amount.toLocaleString("fr-FR")} SP a ${member.tag}. Nouveau total: ${updated.simbaPoints.toLocaleString("fr-FR")} SP.${syncWarning}`
      );
      return;
    }
